/**
 * 
 * @param {String} expression - A string that just contains '+' and '-'
 * @returns {object} { 
 *      op_str - the tstring just with the operation
 *  
 * }
 */
const extractOperationPlusAndMinus = (expression) =>{
    let op_str = expression.match(/[\-\+]?[0-9]+(\.[0-9]+)?[\+\-]{1}[\-\+]?[0-9]+(\.[0-9]+)?/);
    let info = {
        index_start: op_str.index,
    }
    info.index_end = info.index_start + op_str[0].length - 1;
    info.index_op =  info.index_start + op_str[0].slice(1).search(/[\+\-]/) + 1;
    info.sign = expression[info.index_op];
    
    return info;
}

module.exports = extractOperationPlusAndMinus;